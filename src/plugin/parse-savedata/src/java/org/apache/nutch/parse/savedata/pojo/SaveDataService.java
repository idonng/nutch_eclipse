...
public class SaveDataService {
	...
	public void addData(CrawlData sd) {
		...
		try {	
			String sql1="select url from savetest p where p.url=?";
			 sta1 = sdUtils.getConn().prepareStatement(sql1);
			    ...
			    while(re1.next())	
			    {
			    	...
			    	String sql = "update  savetest set fetchtime=? ,title=? , maintext=?,snap=?,pubtime=?,site=?,infotype=?,browsenum=?,commentnum=? ,siteid=? where url=?" ;		
			    	state1 = sdUtils.getConn().prepareStatement(sql);	
			...
			sta = sdUtils.getConn().prepareStatement(sql1);
		    sta.setString(1, sd.getUrl());
		    ResultSet re=sta.executeQuery();
		    
		    while(!re.next())	
		    {
		    	String sql = "insert into savetest(url ,fetchtime ,title , maintext,snap,pubtime,site,infotype,browsenum,commentnum,siteid) values(?,?,?,?,?,?,?,?,?,?,?)" ;		
